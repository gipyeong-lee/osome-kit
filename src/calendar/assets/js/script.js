...
var OsomeCalendar = {
  ...
  createHandler(week, startNum, endNum, eventOption) {
    ...
    _eventHandler.className =
      `event-block-handler-${eventOption.order}-${eventOption.index} resize-handle handler-y`
  ...
  createEvents: function (options) {
    ...
    self.categories.map((category) => {
      ...
      events.map((event, idx) => {
        ...
        const num = utils.convertDateToNumber(event.startDate, event.endDate, currentYear, indexOfCurrentMonth,
          startOfDay, firstTile, endOfMonthDate, self.endNum)
        if (num === undefined) {
          return
        }
  ...
  resetAllEventTotal(order) {
    const self = this
    self.categories[order].events.map(event => {
      event.total = 0
      return event
    })
  },
  ...
  reorderEventBox(order) {
    ...
    for (var scheduleWrapper of scheduleWrappers) {
      ...
      if (children.length > 0) {
        ...
        _weekEl.style.height = `${(eventBlockHeight + eventBlockMargin) * (children.length + 1) +
                                  self.options.style.cellHeader.height + self.options.style.cellHeader.gap}px`
      }
    }
  },
    ...
    onMouseMove: function (self, targetTag) {
      ...
      if (prevWeek === nextWeek) {
        ...
        if (startNum <= currentNumber) {
          self.categories[_order].events[_index] =
            self.syncResizeEvent(_order, _index, _startNum, currentNumber)
  ...
  attachEventCreate: {
    ...
    onMouseUp: function (self, targetTag) {
      if (self.focus.current !== undefined) {
        ...
        if (endNum >= startNum &&
            (start.getAttribute('month') !== null || end.getAttribute('month') !== null)) {
