...
var OsomeGantt = {
  ...
  createHandler(row, startNum, endNum, eventOption) {
    ...
    _eventHandler.style.borderLeft =
      `solid ${halfHeight}px ${eventOption.color || eventOption.style.backgroundColor}`
  ...
  createBlock(row, startNum, endNum, eventOption) {
    ...
    _eventBlock.style.backgroundColor = '#dcdde1'//eventOption.color
    _eventBlock.style.height = `${height}px`
    _eventBlock.style.marginTop = `${gap}px`
  ...
  renderEventBlocks(options) {
    ...
    for (let i = 0; i < _length; i++) {
      ...
      _events.map((event) => {
        const num = utils.convertDateToGanttNumber(event.startDate, event.endDate, currentYear,
          indexOfCurrentMonth, endOfMonthDate)
        if (num === undefined) {
          return
        }
  ...
  renderEventBlock(row, startTile, endTile, eventOption) {
    ...
    _eventText.style.lineHeight = `${height}px`
    ...
    _eventText.style.backgroundColor = eventOption.color
  ...
  createRow: function (type, row, style, content) {
    ...
    if (content !== undefined) {
      ...
      bullet.style.width =
        type === 'main' ? `${self.options.style.row.height / 2}px` : `${self.options.style.row.height / 5}px`
  ...
  isCategoryRow(targetTag) {
    return targetTag.classList.contains('osome-gantt-grid-category-row') || (targetTag.parentElement &&
                                                                             targetTag.parentElement.classList.contains(
                                                                               'osome-gantt-grid-category-row'))
  },
  getCategoryRow(targetTag) {
    if (targetTag.classList.contains('osome-gantt-grid-category-row')) {
      return targetTag
    } else if (targetTag.parentElement &&
               targetTag.parentElement.classList.contains('osome-gantt-grid-category-row')) {
  ...
  isEventBlock(targetTag) {
    return targetTag.classList.contains('event-block') ||
           (targetTag.parentElement && targetTag.parentElement.classList.contains('event-block'))
  },
  ...
  onChangedContainer: function (_leftWidth) {
    ...
    if (leftWidthPercent > self.options.handleMax) {
      leftWidthPercent = self.options.handleMax
    } else if (leftWidthPercent < self.options.handleMin) {
      leftWidthPercent = self.options.handleMin
    }
    self.onCompleteContainerResize(leftWidthPercent, 100 - leftWidthPercent)
  },
  ...
  attachResizeEvent: {
    ...
    onMouseUp: function (self, targetTag) {
      ...
      self.categories[row].events[self.focus.event.index].startDate =
        new Date(self.categories[row].events[self.focus.event.index].startDate)
      self.categories[row].events[self.focus.event.index].endDate =
        new Date(self.categories[row].events[self.focus.event.index].endDate)
      self.saveOffset()
    ...
    onMouseUp: function (self, targetTag) {
      if (self.focus.current !== undefined) {
        ...
        if (start !== undefined && end !== undefined) {
          ...
          const renderOption = {
            startNumber: start.getAttribute('number'),
            endNumber: end.getAttribute('number')
          }
          ...
          // self.attachEvent(row, startNum.toNumber(), endNum.toNumber(), { title:
          // self.categories[row].content.title, detail: 'This is Detail', color:
          // self.categories[row].content.style.color })
        }
      }
    }
  }
